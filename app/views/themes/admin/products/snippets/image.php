<div class="ajax_image">
    ...
    <div class="span10">
        ...
        <?php $hide = (isset($product) AND $product->is_default_image($image->id)) ? 'hide' : ''; ?>
		...
		<?php echo form_dropdown('image['.$image->id.'][type]', array('product' => 'Prodotto', 'gallery' => 'Galleria fotografica', 'environment' => 'Ambientata'), set_value('image['.$image->id.'][type]', $image->get_type())) ?>
		
        <label class="radio">
            <?php echo form_radio('product[default_image_id]', $image->id, set_radio('product[default_image_id]', $image->id, (isset($product)) ? $product->is_default_image($image->id) : TRUE), 'class="default_image"') ?> Immagine prefefinita
        </label>
