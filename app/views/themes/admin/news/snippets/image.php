<div class="ajax_image">
    ...
    <div class="span10">
        ...
        <?php $hide = (isset($news) AND $news->is_default_image($image->id)) ? 'hide' : ''; ?>
		...
		<?php echo form_dropdown('image['.$image->id.'][type]', array('news' => 'Prodotto', 'technical' => 'Tecnica', 'environment' => 'Ambientata'), set_value('image['.$image->id.'][type]', $image->get_type())) ?>
		*/ ?>
		
        <label class="radio">
            <?php echo form_radio('news[default_image_id]', $image->id, set_radio('news[default_image_id]', $image->id, (isset($news)) ? $news->is_default_image($image->id) : TRUE), 'class="default_image"') ?> Immagine prefefinita
        </label>
