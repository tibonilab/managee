    ...
    <div class="span10">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane <?php echo $active ?>" id="image_<?php echo $image->id ?>_<?php echo $iso ?>">
				...
				<?php echo form_label('Etichetta') ?>
                <?php echo form_input('image[content]['.$image->id.']['.$iso.'][label]', set_value('image[content]['.$image->id.']['.$iso.'][label]', $image->get_contents($iso)->label)) ?>
				
                <?php echo form_label('Titolo') ?>
				...
				<?php echo form_label('Href') ?>
                <?php echo form_input('image[content]['.$image->id.']['.$iso.'][href]', set_value('image[content]['.$image->id.']['.$iso.'][href]', $image->get_contents($iso)->href)) ?>
            </div>
            <?php $active = '';endforeach ?>
        </div>
        ...
        <?php $hide = (isset($showcase) AND $showcase->is_default_image($image->id)) ? 'hide' : ''; ?>
        ...
        <label class="radio">
            <?php echo form_radio('showcase[default_image_id]', $image->id, set_radio('showcase[default_image_id]', $image->id, (isset($showcase)) ? $showcase->is_default_image($image->id) : FALSE), 'class="default_image"') ?> Immagine prefefinita
        </label>
